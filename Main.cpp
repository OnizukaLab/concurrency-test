...
int main(int argc, char **argv){  
  ...
  switch(kind){
    case 0:{
      auto inc0 = new HashIncNoCtrl(niters, conc, load, len, dens, chunk, w_rate);
      auto res0 = inc0->go();
      cout << res0.count() << endl;
      break;
    }
    case 1:{
      auto inc1 = new HashIncMutex(niters, conc, load, len, dens, chunk, w_rate);
      auto res1 = inc1->go();
    ...
    case 4:{
      auto inc4 = new HashIncHLE(niters, conc, load, len, dens, chunk, w_rate);
      auto res4 = inc4->go();
      cout << res4.count() << endl;
    }
    case 5:{
      auto inc5 = new HashIncRTM(niters, conc, load, len, dens, chunk, w_rate);
      auto res5 = inc5->go();
      cout << res5.count() << endl;
    }
    case 6:{
      auto inc6 = new HashIncAtomic(niters, conc, load, len, dens, chunk, w_rate);
      auto res6 = inc6->go();
